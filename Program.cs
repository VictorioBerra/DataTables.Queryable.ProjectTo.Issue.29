        ...
        public static async Task Main(string[] args)
        {
            ...
            using (host)
            {
                ...
                using (var context = host.Services.GetService<MyAppContext>())
                {
                    ...
                    var queryable = context.Cat;//.AsQueryable();

                    var viewmodel = new DataTablesAjaxPostModel()
                    {
                        Draw = 1,
                        Length = 50,
                        Columns = new List<DataTablesAjaxPostModel.ColumnData>(),
                        Order = new List<DataTablesAjaxPostModel.OrderData>(),
                        Search = new DataTablesAjaxPostModel.SearchData()
                        {
                            Value = String.Empty
                        }
                    };

                    var request = new DataTablesRequest<Cat>(viewmodel);

                    var results = queryable
                        .Filter(request)
                        .ProjectTo<CatViewModel>(Mapper.Configuration)
                        .ToList(); 
